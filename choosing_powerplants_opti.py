...
selected_power_plants = pd.read_excel('RawData/selected_plants.xlsx',index_col=None,header=None)

this =selected_power_plants[0].tolist()
used_plant_units = []
for plant in plant_units:

    if plant.name in this:
        used_plant_units.append(plant)
...
scheduling_time_blocks, scheduling_dates, plant_names, plant_production_costs,plant_thermal_effeciencies, plant_upper_capacity,plant_lower_capacity,plant_ramp_up_deltas,plant_ramp_down_deltas = reading_optimization_data(used_plant_units,demand_UP)
...
opti_solution_location = OUTPUT_SOLUTION_FOLDER +"/"+ MONTH + "__opti_with_selected_plants_solution_"+DEMAND_PROFILE+"_"+str(datetime.now().date())+".csv"
