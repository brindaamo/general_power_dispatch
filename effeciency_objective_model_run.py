...
import pandas as pd
from utils import get_only_thermal_power_plant, get_plant_characteristics,demand_with_only_thermal,get_thermal_effeciency, get_raw_data_by_time,reading_input_data,get_demand_data,assign_hydro_as_peak,get_demand_based_on_profile,add_new_plants
from utils import INPUT_RAW_FILE_NAME,INPUT_THERMAL_EFFECIENCY_FILE_NAME,DEVELOPMENT_PERIOD_END_TIME,DEVELOPMENT_PERIOD_START_TIME,MODEL_PERIOD_END_TIME,MODEL_PERIOD_START_TIME,OUTPUT_ACTUALS_FOLDER,OUTPUT_SOLUTION_FOLDER,MONTH,DEMAND_PROFILE
...
from output_validations import output_formatting,add_plant_percent_ramp_up_or_down, output_plant_chars
#---------------------data preprocessing-------------------------
...
#getting demand data 
demand_of_UP_bydate_byhour_units,demand_UP = demand_with_only_thermal(model_data)
...
scheduling_hours, scheduling_dates, plant_names, plant_production_costs,plant_thermal_effeciencies,plant_upper_capacity,plant_lower_capacity,plant_ramp_up_deltas,plant_ramp_down_deltas = reading_optimization_data(thermal_plants,demand_UP)
...
capacity_constraint_checks_to_json = pd.read_json(output_plant_chars_add)


opti_solution_location = OUTPUT_SOLUTION_FOLDER +"/"+ MONTH + "__eff_opti_solution_"+DEMAND_PROFILE+"_"+str(datetime.now().date())+".csv"
capacity_constraint_checks_to_json.to_csv(opti_solution_location,index=False)
